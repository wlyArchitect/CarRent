...
<script type="text/javascript">
    ...
    layui.extend({
        ...
        tableIns=table.render({
            ...
            ,url:'${whContextPath}/user/loadAllUser' //数据接口
        ...
        $("#doSearch").click(function(){
            ...
            alert(params);
            tableIns.reload({
                url:"${whContextPath}/user/loadAllUser?"+params
            })
        });
        ...
        table.on('tool(userTable)', function(obj){
            ...
            if(layEvent === 'del'){ //删除
                layer.confirm('真的删除【'+data.realname+'】这个用户吗', function(index){
                    //向服务端发送删除指令
                    $.post("${whContextPath}/user/deleteUser",{userid:data.userid},function(res){
                        layer.msg(res.msg);
                        //刷新数据 表格
                        tableIns.reload();
                    })
                ...
                layer.confirm('真的重置【'+data.realname+'】这个用户的密码吗', function(index){
                    //向服务端发送删除指令
                    $.post("${whContextPath}/user/resetUserPwd",{userid:data.userid},function(res){
                        layer.msg(res.msg);
                    })
                });
        ...
        function openAddUser(){
            mainIndex=layer.open({
                ...
                success:function(index){
                    //清空表单数据
                    $("#dataFrm")[0].reset();
                    url="${whContextPath}/user/addUser";
                }
            });
        }
        //打开修改页面
        function openUpdateUser(data){
            mainIndex=layer.open({
                ...
                success:function(index){
                    ...
                    url="${whContextPath}/user/updateUser";
                }
            });
        }
        //保存
        ...
        function deleteBatch(){
            ...
            layer.confirm('真的删除选中的这些用户吗', function(index){
                //向服务端发送删除指令
                $.post("${whContextPath}/user/deleteBatchUser",params,function(res){
                    layer.msg(res.msg);
                    //刷新数据 表格
                    tableIns.reload();
                })
            ...
            mainIndex=layer.open({
                ...
                success:function(index){
                    ...
                    var roleTableIns=table.render({
                        ...
                        ,url:' ${whContext}/user/initUserRole?userid='+data.userid //数据接口
                        ,title: '角色列表'//数据导出来的标题
                        ,cellMinWidth:100 //设置列的最小默认宽度
                        ,cols: [[   //列表数据
                            {type: 'checkbox', fixed: 'left'}
