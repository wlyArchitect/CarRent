...
<script type="text/javascript">
    ...
    layui.use([ 'jquery', 'layer', 'form', 'table','laydate','layedit' ], function() {
        ...
        tableIns=table.render({
            ...
            ,url:'${whContextPath}/news/loadAllNews' //数据接口
        ...
        $("#doSearch").click(function(){
            ...
            tableIns.reload({
                url:"${whContextPath}/news/loadAllNews?"+params,
        ...
        table.on('tool(newsTable)', function(obj){
            ...
            if(layEvent === 'del'){ //删除
                layer.confirm('真的删除【'+data.title+'】这个公告吗', function(index){
                    //向服务端发送删除指令
                    $.post("${whContextPath}/news/deleteNews",{id:data.id},function(res){
                        layer.msg(res.msg);
                        //刷新数据 表格
                        tableIns.reload();
                    })
        ...
        function openAddNews(){
            mainIndex=layer.open({
                ...
                success:function(index){
                    ...
                    //清空表单数据
                    $("#dataFrm")[0].reset();
                    url="${whContextPath}/news/addNews";
                }
            });
        }
        //重置按钮
        ...
        function openUpdateNews(data){
            mainIndex=layer.open({
                ...
                area:['800px','550px'],
                success:function(index){
                    //建立编辑器 将文本域id名为content的内容给富文本编辑器
                    editIndex=layedit.build('content');
                    form.val("dataFrm",data);
                    url="${whContextPath}/news/updateNews";
                }
            });
        }
        //todo 保存(细节)
        form.on("submit(doSubmit)",function(obj){
            //把富文本编辑器 里面的数据同步到自己写的<textarea>里面
        ...
        function deleteBatch(){
            ...
            layer.confirm('真的删除选中的这些公告吗', function(index){
                //向服务端发送删除指令
                $.post("${whContextPath}/news/deleteBatchNews",params,function(res){
                    layer.msg(res.msg);
                    //刷新数据 表格
                    tableIns.reload();
                })
